    ...
    def callEVM(self, dumpedType, dumpedData, dumpedFrom, dumpedDest,dumedSignedDatabyDevice,dumpedDevPubKey):
        ...
        try:
            ...
            if respostaJSON['Erro'] != "":
                ...
                logSignSize.append("SizeSignature;"+ signatureAlgoritm+ ";" + str(signSize) + ";Bytes" +";callEVM")
                print("size logged")
...
#############################################################################
            ...
            while (len(candidatePool) > 0):
                ...
                if (candidateTransaction != False):
                    ...
                    if(ChainFunctionsMulti.findBlock(devPublicKey)!=False):
                        ...
                        print("sign logged")
                        signSize = len(signData)
                        logSignSize.append("SizeSignature;"+ signatureAlgoritm+ ";" + str(signSize) + ";Bytes" + ";lifecycleEvent transaction " + str(nextInt) + ";block " + str(blk.index))
                        print("size logged")
                        gwTime = "{:.0f}".format(((time.time() * 1000) * 1000))
                        # code responsible to create the hash between Info nodes.
    ...
    def votePoolCandidateMulti(self, context, candidatePool, pickedGwPub):
        ...
        print("sign logged")
        signSize = len(votesSignature)
        logSignSize.append("SizeSignature;"+ signatureAlgoritm+ ";" + str(signSize) + ";Bytes" + ";current vote pool multi")
        print("size logged")
        t2 = (time.time()*1000)
        logT23.append("T23 VOTING;CONTEXT "+context+";VOTING TIME; " + str(t2-t1))
        # logger.error("!!!!! My verification sign = " + str(CryptoFunctions.signVerifyECDSA(str(votesPool),votesSignature,gwPub)))
